...
### 2019.05.13
 ...
 > - А в чем здесь конкретно сложность? Может мы переделаем дизайн на что-то более простое. Просто не совсем понимаю, в чем проблема.

Мало какие css-свойства блока/тега "option" доступны для редактирования (в-основном только шрифты и т.п.).
Это либо нужно подключать дополнительные библиотеки (что нерационально для такоё мелочи), либо городить что-то самопальное, что будет выглядеть как "select>option" (что довольно трудоёмко).

Изначально ничего этого не было в дизайне, разумеется, потому и не обратил на это внимание.
 
 > 2) Странно. Вижу, что правда в коде так написано.
 > Просто если в самой фигме смотреть не где код, а где дизайнеры выбирают шрифты, там Roboto Light: https://monosnap.com/file/sdJPuRDWbgzbPlmJi8mUwE1iawIlxa 

Если вы скинете файлы шрифтов, то смогу использовать любые. 

 > Сейчас еще почему-то поиск и кнопка поехали: https://monosnap.com/file/6l4aVx9VrYLxxYvPnZdCzY3iT6ZYK3 

Исправлено.

 > В адаптиве календарь тоже поехал (и в десктопе тоже немного): https://monosnap.com/file/DNzl5385r8c3k3c8a5lJ90XLPIR0UI 
